...
__all__ = ["LatteWithLoss"]

logger = logging.getLogger(__name__)



class LatteWithLoss(nn.Cell):
    ...
    def __init__(
        ...
        self.broadcast_t = None if not get_sequence_parallel_state() else \
            ops.Broadcast(root_rank=hccl_info.group_id * hccl_info.world_size, group=hccl_info.group)
...
class DiffusionWithGrad(nn.Cell):
    ...
    def __init__(
        ...
        use_image_num: int = 0,
        dtype=ms.float32,
        optimizer: nn.Cell = None,
        scale_sense: float = 1.0,
        ema: nn.Cell = None,
        drop_overflow_update: bool = True,
        gradient_accumulation_steps: bool = 1,
        clip_grad: bool = False,
        clip_norm: bool = 1.0,
    ):
        super().__init__()

        assert not cond_stage_trainable

        latte_with_loss = LatteWithLoss(network, diffusion, use_image_num, dtype)
        latte_with_grad = TrainOneStepWrapper(
            latte_with_loss,
            optimizer=optimizer,
        ...
        self.latte_with_grad = latte_with_grad
        self.scale_sense = latte_with_grad.scale_sense
    ...
    def construct(
        ...
        loss, cond, scaling_sens = self.latte_with_grad(x, text_embed, encoder_attention_mask, attention_mask)
