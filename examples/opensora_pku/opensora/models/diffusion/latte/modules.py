        ...
        if self.enable_flash_attention:
            if self.layout == "SBH":
                ...
                self.flash_attention = MSFlashAttention(
                    head_dim=dim_head, head_num=heads // hccl_info.world_size, fix_head_dims=[72], attention_dropout=attn_drop, dtype=ms.bfloat16
                )
            else:
                self.flash_attention = MSFlashAttention(
                    head_dim=dim_head, head_num=heads, fix_head_dims=[72], attention_dropout=attn_drop, dtype=ms.bfloat16
                )
        else:
