        ...
        if self.enable_flash_attention:
            if self.layout == "SBH":
                ...
                self.flash_attention = MSFlashAttention(
                    head_dim=dim_head, head_num=heads // hccl_info.world_size, fix_head_dims=[72], attention_dropout=attn_drop
                )
            else:
                self.flash_attention = MSFlashAttention(
...
class LatteT2VBlock(nn.Cell):
    ...
    def construct(
        ...
        if enable_temporal_attentions:
            ...
            if get_sequence_parallel_state():
                # (b * frame/sp, h*w, c) -> (b, frame/sp, h*w, c) -> (frame/sp, b, h*w, c)
                hidden_states = hidden_states.view(
                    input_batch_size, frame + use_image_num, num_patches, -1).swapaxes(0, 1).contiguous()

                # (frame/sp, b, h*w, c) -> (frame/sp, b*h*w, c)
                ...
                # (frame/sp, b * h*w, c) -> (b * frame/sp, h*w, c)
                # 'f (b t) d -> (b f) t d'
                f, _, d = hidden_states.shape
                hidden_states = hidden_states.view(
                    f, input_batch_size, -1, d).transpose(1, 0, 2, 3).view(input_batch_size * f, -1, d).contiguous()
