...
from opensora.models.diffusion.latte.net_with_grad import DiffusionWithGrad
...
def main(args):
    ...
    if get_sequence_parallel_state():
        ...
        logger.info(f"optimizer parallelism: adamw shard size setting to {_shard_size}")
    else:
        optimizer = AdamW(
            latte_model.trainable_params(),
            learning_rate=lr,
    ...
    net_with_grads = DiffusionWithGrad(
        latte_model,
        diffusion,
        vae=vae,
        condition="text",
