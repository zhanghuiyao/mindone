...
from gm.modules.attention import SpatialTransformer, SpatialTransformerDenseDiff
...
class TimestepEmbedSequentialDenseDiff(nn.SequentialCell, TimestepBlock):
    def construct(self, x, emb, context=None, **kwargs):
        for cell in self.cell_list:
            if isinstance(cell, TimestepBlock):
                x = cell(x, emb)
                ...
                x = cell(x, context)
            elif isinstance(cell, SpatialTransformerDenseDiff):
                x = cell(x, context, **kwargs)
            else:
                x = cell(x)
        return x


class UNetModelDenseDiff(nn.Cell):
    def __init__(
        self,
        in_channels,
        model_channels,
        ...
        self.input_blocks = nn.CellList(
            [TimestepEmbedSequentialDenseDiff(conv_nd(dims, in_channels, model_channels, 3, padding=1, pad_mode="pad"))]
        )
        self._feature_size = model_channels
        input_block_chans = [model_channels]
        ch = model_channels
        ...
        for level, mult in enumerate(channel_mult):
            for nr in range(self.num_res_blocks[level]):
                ...
                self.input_blocks.append(TimestepEmbedSequentialDenseDiff(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)
            if level != len(channel_mult) - 1:
                out_ch = ch
                self.input_blocks.append(
                    TimestepEmbedSequentialDenseDiff(
                        ResBlock(
                            ch,
                            time_embed_dim,
                            dropout,
        ...
        if legacy:
            # num_heads = 1
            dim_head = ch // num_heads if use_spatial_transformer else num_head_channels
        self.middle_block = TimestepEmbedSequentialDenseDiff(
            ResBlock(
                ch,
                time_embed_dim,
                dropout,
            ...
            SpatialTransformerDenseDiff(  # always uses a self-attn
                ch,
                num_heads,
                dim_head,
                depth=transformer_depth_middle,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(self.num_res_blocks[level] + 1):
                ...
                if level and i == self.num_res_blocks[level]:
                    ...
                    ds //= 2
                self.output_blocks.append(TimestepEmbedSequentialDenseDiff(*layers))
                self._feature_size += ch
