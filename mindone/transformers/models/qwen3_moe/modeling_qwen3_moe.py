...
from mindone.transformers.modeling_utils import ALL_ATTENTION_FUNCTIONS, PreTrainedModel
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    attn_weights = mint.matmul(query, key_states.swapdims(2, 3)) * scaling
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask

    attn_weights = mint.softmax(attn_weights, dim=-1, dtype=mindspore.float32).to(query.dtype)
...
class Qwen3MoeAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_states = self.q_norm(self.q_proj(hidden_states).view(hidden_shape)).swapdims(1, 2)
        key_states = self.k_norm(self.k_proj(hidden_states).view(hidden_shape)).swapdims(1, 2)
        value_states = self.v_proj(hidden_states).view(hidden_shape).swapdims(1, 2)
...
class Qwen3MoeSparseMoeBlock(nn.Cell):
    ...
    def construct(self, hidden_states: mindspore.Tensor) -> mindspore.Tensor:
        ...
        routing_weights = mint.softmax(router_logits, dim=1, dtype=mindspore.float32)
...
class Qwen3MoeRotaryEmbedding(nn.Cell):
...
QWEN3_MOE_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length) or `BlockMask`, *optional*):
...
class Qwen3MoeForQuestionAnswering(Qwen3MoePreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,
        ...
        outputs: tuple = self.transformer(
            input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
