...
# FIXME: replace to mindone.mindspore_adapter.attention.scaled_dot_product_attention
from .scaled_dot_product_attn import scaled_dot_product_attention
...
class Qwen2MoeRotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, position_ids):
        ...
        return ops.stop_gradient(cos.to(dtype=x.dtype)), ops.stop_gradient(sin.to(dtype=x.dtype))
...
class Qwen2MoeFlashAttention2(Qwen2MoeAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        raise NotImplementedError(f"flash attention do not support on qwen2_moe model now.")
...
class Qwen2MoeSparseMoeBlock(nn.Cell):
    ...
    def construct(self, hidden_states: mindspore.Tensor) -> mindspore.Tensor:
        ...
        # FIXME: original code format, not support on MindSpore 2.5.0
        #1. original
        ...
        shared_expert_output = mint.nn.functional.sigmoid(self.shared_expert_gate(hidden_states)) * shared_expert_output
...
class Qwen2MoeForSequenceClassification(Qwen2MoePreTrainedModel):
    ...
    def construct(
        ...
        transformer_outputs: tuple = self.model(
            input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
        ...
        result = (loss, pooled_logits) + transformer_outputs[1:]
        result = tuple(v for v in result if v is not None)
        return result
...
class Qwen2MoeForQuestionAnswering(Qwen2MoePreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,
        ...
        result = (loss, start_logits, end_logits) + outputs[1:]
        result = tuple(v for v in result if v is not None)
        return result
